...
zip_ :: Zippable f => f a -> f b -> f (a,b)
zip_ = zipWith_ (,)
{-# INLINE zip_ #-}

zip3_ :: Zippable f => f a -> f b -> f c -> f (a,b,c)
zip3_ = zipWith3_ (,,)
{-# INLINE zip3_ #-}

zip4_ :: Zippable f => f a -> f b -> f c -> f d -> f (a,b,c,d)
zip4_ = zipWith4_ (,,,)
{-# INLINE zip4_ #-}

zip5_ :: Zippable f => f a -> f b -> f c -> f d -> f e -> f (a,b,c,d,e)
zip5_ = zipWith5_ (,,,,)
{-# INLINE zip5_ #-}

zip6_ :: Zippable f => f a -> f b -> f c -> f d -> f e -> f g -> f (a,b,c,d,e,g)
zip6_ = zipWith6_ (,,,,,)
{-# INLINE zip6_ #-}
