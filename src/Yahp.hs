...
lookupThrow' :: (ConvertText Text e, MonadError e m, Ixed (t a)
                ...
                Int -> Text -> Index (t a) -> t a -> m (IxValue (t a))
lookupThrow' showKeys msg k c = maybeThrow (toS $ err <> msg) $ c ^? ix k
  where err =  shot k <> " (type: " <> shot (typeOf k) <> ") not found in container. " <> case showKeys of
          0 -> "Count " <> shot len
          n  -> let lim = bool "" (" (showing " <> shot n <> " of " <> shot len <> ")") $ n < len
...
lookupThrow = lookupThrow' maxBound ("" :: Text)
{-# INLINE lookupThrow #-}
...
groupByMap :: (Foldable f, Ord a) => (b -> a) -> f b -> ML.Map a (NE b)
groupByMap f = fmap (fromList . D.toList) . ML.fromListWith (<>) . fmap (f &&& D.singleton) . toList
{-# INLINABLE groupByMap #-}

groupByHashMap :: (Foldable f, Eq a, Hashable a) => (b -> a) -> f b -> HML.HashMap a (NE b)
groupByHashMap f = fmap (fromList . D.toList) . HML.fromListWith (<>) . fmap (f &&& D.singleton) . toList
{-# INLINABLE groupByHashMap #-}

groupMap :: (Foldable f, Ord a) => f (a,b) -> ML.Map a (NonEmpty b)
groupMap = fmap (fromList . D.toList) . ML.fromListWith (<>) . fmap2 D.singleton . toList
{-# INLINABLE groupMap #-}

groupHashMap :: (Eq a, Foldable f, Hashable a) => f (a,b) -> HML.HashMap a (NE b)
groupHashMap = fmap (fromList . D.toList) . HML.fromListWith (<>) . fmap2 D.singleton . toList
...
hmFromListTellDups xs = HM.traverseWithKey g $ (groupHashMap xs :: HM.HashMap a (NonEmpty v))
  where g _ (x:|[])     = pure x
...
mFromListTellDups :: forall a t v m. (MonadWriter [Text] m, Typeable a, Ord a, Show a, Foldable t)
  => t (a,v) -> m (M.Map a v)
mFromListTellDups xs = M.traverseWithKey g $ (groupMap xs :: M.Map a (NonEmpty v))
  where g _ (x:|[])     = pure x
        g k (x:|r)      = x <$ tell ["Key (" <> showt (typeOf k) <> ") " <> shot k <> " appears "
                                     <> showt (succ $ length r) <> " times" :: Text]
        g :: a -> NonEmpty v -> m v
