...
import           Yahp.FinancialUnits as S (showFinancialValue)
...
lookupThrow'
  :: (MonadError e m, ConvertText a1 String, ConvertText String e,
      Ixed (t a2), Typeable (Index (t a2)),
      Show (Index (t a2)), Foldable t) =>
     a1 -> Index (t a2) -> t a2 -> m (IxValue (t a2))
lookupThrow' msg k c = maybeThrow (toS $ err <> toS msg) $ c ^? ix k
...
lookupThrow :: (MonadError e m, ConvertText String e,
                 At (t a2), Typeable (Index (t a2)),
                 Show (Index (t a2)), Foldable t) =>
                Index (t a2) -> t a2 -> m (IxValue (t a2))
lookupThrow = lookupThrow' ("" :: String)
{-# INLINE lookupThrow #-}
...
hmToList :: HM.HashMap k v -> [(k, v)]
hmToList = HM.toList
{-# INLINE hmToList #-}

mToList :: M.Map k a -> [(k, a)]
mToList = M.toList
{-# INLINE mToList #-}
