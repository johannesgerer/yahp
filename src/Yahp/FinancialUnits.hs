...
showThousandsSeparatorUS :: Roundable a => Int -> a -> Text
showThousandsSeparatorUS = showThousandsSeparator ',' '.'

-- | Copyright (c) 2017 Alexander Thiemann
showThousandsSeparator :: (Roundable a) => Char -> Char -> Int -> a -> Text
...
showFinancialUnits decimals val = foldr g (pick $ last scaledValues)
...
instance {-# OVERLAPPABLE #-} (Ord a, Num a, RealFrac a) => Roundable a where
